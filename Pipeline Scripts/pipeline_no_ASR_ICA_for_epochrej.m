...
while flag == 0
    ...
    if strcmp(rejectionMethod,'cleanrawdata')
        ...
        removedChans = strjoin({EEG.chanlocs(~ismember({originalEEG.chanlocs(:).labels},...
...
EEG.comments = pop_comments(EEG.comments,'',['Rejected epochs ', strjoin(string(find(~ismember(events1,events2))),', ')],1);
find(~ismember(events1,events2))
...
EEG.comments = pop_comments(EEG.comments,'','Ran ICA',1);
...
uiwait
EEG = pop_subcomp(EEG);
EEG.comments = pop_comments(EEG.comments,'',['Rejected indepdent components ', ...
...
[ALLEEG EEG CURRENTSET] = pop_newset(ALLEEG, EEG, 1,'setname',strcat(name,'_ica_rejected'),...
    'savenew',strcat(fileDir, filesep,name,'_ica_rejected'),'overwrite','on','gui','off'); 
EEG = eeg_checkset(EEG);
