...
function eeglab2loreta( chanlocs, winv, varargin )
    ...
    g = finputcheck( varargin, { 'fileloc'   'string'     []      'loreta_chanlocs';
                                 ...
                                 'savingDir'  'string'    []      []}); % modified by Danielle
    ...
    if strcmpi(g.labelonly, 'off')
        ...
        % writing file
        % ------------
        fid = fopen( [g.savingDir, filesep, g.fileloc '.xyz' ], 'w'); % modified by Danielle
        fprintf(fid, '%d\n', length(chanlocs));
        ...
        % export labels only
        % ------------------
        fid = fopen( [g.savingDir, filesep, g.fileloc '.txt' ], 'w'); % modified by Danielle
        for index = 1:length(chanlocs)
    ...
    if ~isempty(winv)
        if strcmpi(g.exporterp, 'off')
            for iComp=1:length(g.compnum)
                ...
                save('-ascii', strcat(g.savingDir,filesep,filename), 'toLORETA'); % modified by Danielle
                fprintf('File %s saved\n', filename);
            end
        else
